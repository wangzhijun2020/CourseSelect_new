    ...
    begin
      ...
      if not [".xls", ".xlsx"].include?(file_ext)
        ...
        (2..spreadsheet.last_row).each do |i|
          ...
          @grade.update_attributes!(:grade => spreadsheet.row(i)[7])
        end
        flash={:success => "上传成功"}
      end
      redirect_to grades_path(course_id: params[:course_id]), flash: flash
  ...
  def export
    ...
    respond_to do |format|
      ...
      format.xls {
        ...
        sheet1.row(0).concat %w{成绩id 学号 姓名 专业 课程名 学时学分 授课老师 成绩}
        @grades.each_with_index do |grade,i|
          #puts(grade.user.num)
          sheet1.row(i+1).push grade.id, grade.user.num, grade.user.name, grade.user.major, grade.course.name, grade.course.credit, grade.course.teacher.name, grade.grade
        end
